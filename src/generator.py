...
#modified to perform the augmentation
def add_numeric_column_modified(df, length, label="Sig_id", dist=None, vals=None):
    print("adding numeric column")
    spt = 0
    if dist is None or dist == "range":
        generated_vals = [num for num in tqdm(range(length))]  # pylint: disable=R1721
        return generated_vals

    elif dist == "normal":
        start = time.time()
        generated_vals = stats.norm.rvs((vals[0] + vals[1])//2, abs(vals[0] - vals[1])//6, length)
        return generated_vals
...
def add_categorical_column_modified(df, length, label, values, options=None, weights=None):
    print("adding categorical column")
    if options is None:
        df[label] = values
    elif weights is None or len(weights) == 0:
        arr = [secrets.choice(options) for i in tqdm(range(length))]
        return arr
    else:
...
def add_noise_modified(og_sig):  # pylint: disable=W010
    mean=np.mean(og_sig)
    std_dev=np.std(og_sig)
    maximum=np.max(og_sig)
    degree=len(str(maximum))
    final_degree=1
    for i in range(degree):
        final_degree=final_degree*10
    mean=mean//final_degree
    std_dev=std_dev//final_degree
    gaussian_noise = np.random.normal(mean, std_dev, len(og_sig))
    samples=[]
    for i in range(len(og_sig)):
        samples.append(og_sig[i]+gaussian_noise[i])
    return samples
